...
function Test-CreationStagingCurrent {
    $contractPath = Join-Path $stagingRoot "component-delivery\windows\creation-v1.json"
    $distRoot = Join-Path $repoRoot "src\overlay\three_d_generator\dist"
    if (-not (Test-Path -LiteralPath $contractPath -PathType Leaf) -or
        -not (Test-Path -LiteralPath $distRoot -PathType Container)) {
        return $false
    }

    try {
        $contract = Get-Content -LiteralPath $contractPath -Raw | ConvertFrom-Json
        $webFiles = @($contract.windows.files | Where-Object { $_.path -like "web/*" })
        if ($webFiles.Count -eq 0) {
            return $false
        }
        $contractPaths = @($webFiles | ForEach-Object { $_.path.Substring(4) } | Sort-Object)
        $localPaths = @(Get-ChildItem -LiteralPath $distRoot -File -Recurse | ForEach-Object {
            [IO.Path]::GetRelativePath($distRoot, $_.FullName).Replace("\", "/")
        } | Sort-Object)
        if (($contractPaths -join "`n") -ne ($localPaths -join "`n")) {
            return $false
        }
        foreach ($file in $webFiles) {
            $relative = $file.path.Substring(4).Replace("/", [IO.Path]::DirectorySeparatorChar)
            $localPath = Join-Path $distRoot $relative
            $local = Get-Item -LiteralPath $localPath -ErrorAction Stop
            if ($local.Length -ne [long]$file.sizeBytes) {
                return $false
            }
...
function Sync-CreationStaging {
    if (Test-CreationStagingCurrent) {
        Write-Host "Creation staging already matches the built frontend." -ForegroundColor Green
        return
    }
    Write-Section "Refreshing Creation staging delivery"
    & (Join-Path $repoRoot "scripts\build-component-candidate.ps1") `
        -Component creation -Stage -SkipNpmInstall -SkipCacheMaintenance
    if ($LASTEXITCODE -ne 0) {
        throw "Creation staging refresh failed"
    }
    if (-not (Test-CreationStagingCurrent)) {
        throw "Creation staging does not match the built frontend after refresh"
    }
}
