#!/usr/bin/env python3
"""Verify staged web packs on the immutable GitHub release and emit host delivery data."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--packages",
        default="local-runtime-bundles/sgt_web_assets/sgt_web_assets.packages.json",
    )
    parser.add_argument(
        "--output",
        default="local-runtime-bundles/sgt_web_assets/sgt_web_assets.delivery.json",
    )
    args = parser.parse_args()
    repo = Path(__file__).resolve().parents[1]
    packages_path = require_repo_or_managed_cache(
        repo, repo / args.packages, "web-asset package manifest"
    )
    output_path = require_repo_or_managed_cache(
        repo, repo / args.output, "web-asset delivery manifest"
    )
