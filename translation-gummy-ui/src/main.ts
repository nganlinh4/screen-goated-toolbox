...
function renderTranscripts(payload: TranslationGummyState) {
  ...
  if (!state.firstDetectedLang) {
    for (const e of entries) {
      ...
      if (e.lang && e.output) {
        state.firstDetectedLang = e.lang;
        break;
      }
    }
  }
  ...
  // Build a set of current entry IDs for cleanup
  const currentIds = new Set(entries.map(e => `e${e.id}`));

  // Remove stale DOM nodes (including empty-state placeholder)
  for (const child of Array.from(el.transcriptBody.children)) {
    const domId = (child as HTMLElement).dataset.eid;
    if (!domId || !currentIds.has(domId)) child.remove();
  }

  // Upsert each entry — update in place or append
  let prevNode: Element | null = null;
  let addedPill = false;
  for (const entry of entries) {
    const eid = `e${entry.id}`;
    let node = el.transcriptBody.querySelector(`[data-eid="${eid}"]`);
    ...
    if ("type" in entry && entry.type === "separator") {
      if (!node) {
        ...
        insertAfter(el.transcriptBody, node, prevNode);
      }
      prevNode = node;
      continue;
    }
    ...
    if (!node) {
      ...
      if (el2.className !== wantClass) {
        if (el2.classList.contains("msg-center") && align !== "msg-center") {
          requestAnimationFrame(() => {
            requestAnimationFrame(() => {
              if (el2.isConnected) animateClassChange(el2, wantClass);
            });
          });
        } else {
          animateClassChange(el2, wantClass);
        }
      }
      const content = el2.querySelector<HTMLElement>(".pill-content");
      if (content) {
        content.innerHTML = "";
        ...
        el2.innerHTML = buildPillInner(pair);
      }
    }
    prevNode = node;
  }
