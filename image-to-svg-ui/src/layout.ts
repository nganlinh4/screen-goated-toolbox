...
<section class="shell">
  ...
  <header class="titlebar" id="dragRegion">
    <div class="identity"><span class="app-icon">${icon.vector}</span><strong>${t("title")}</strong><span class="readiness" id="readiness"><i></i><span id="readyText">${t("preparing")}</span></span></div>
  ...
  <main class="workspace">
    <aside class="queue-rail">
      <div class="rail-heading"><span>${t("queue")}</span><button class="icon-button add" id="addImages" title="${t("addImages")}">${icon.add}</button></div>
    ...
    <section class="stage">
      <div class="artboard-wrap">
        ...
        <div class="canvas-toolbar" id="viewerToolbar" hidden><button class="view-button" id="zoomOut" title="${t("zoomOut")}">${icon.zoomOut}</button><output id="zoomValue">100%</output><button class="view-button" id="zoomIn" title="${t("zoomIn")}">${icon.zoomIn}</button><button class="view-button" id="fitView" title="${t("fitView")}">${icon.fit}</button><i></i><button class="view-button" id="canvasBackground" title="${t("canvasBackground")}">${icon.checker}</button><button class="view-button" id="showOutlines" title="${t("showOutlines")}">${icon.outline}</button></div>
