...
pub(super) fn get() -> DesktopSettingsLocaleText {
    DesktopSettingsLocaleText {
        ...
        preset_model_update_title: "새 프리셋 모델 설정을 적용할까요?",
        preset_model_update_body: "이번 업데이트에는 일부 프리셋에 권장되는 최신 모델이 포함되어 있습니다. 적용하면 해당 프리셋의 모델 필드만 변경되며, 프롬프트, 그래프, 모드, 즐겨찾기, 단축키 및 그 밖의 모든 설정은 그대로 유지됩니다. 모델 우선순위 목록도 새 기본값으로 업데이트됩니다. 건너뛰어도 나중에 모든 프리셋 또는 개별 프리셋을 복원하여 새 기본값을 적용할 수 있습니다.",
