...
pub(super) fn get() -> DesktopSettingsLocaleText {
    DesktopSettingsLocaleText {
        ...
        preset_model_update_body: "This update includes newer recommended models for some built-in preset blocks. Applying it changes only the model field, and only for blocks in any profile that still use the previous default model. Prompts, graphs, modes, favorites, hotkeys, and every other setting stay unchanged. If you skip, you can get the new defaults later by restoring built-in presets or an individual preset.",
