...
pub(super) fn speak_settings_preview(text: &LocaleText, speaker_name: &str) {
    if !text.tts_settings.tts_preview_texts.is_empty() {
        ...
        let len = text.tts_settings.tts_preview_texts.len();
        ...
        let preview_text = text.tts_settings.tts_preview_texts[idx].replace("{}", speaker_name);
