...
impl From<TtsProfileWire> for TtsRequestProfile {
    fn from(wire: TtsProfileWire) -> Self {
        ...
        let edge_voice_fallback = edge_voice_configs
            .first()
            .map(|config| config.voice_name.clone())
            .unwrap_or(defaults.edge_voice);
        ...
        Self {
            ...
            edge_voice: trimmed_or(wire.edge_voice, edge_voice_fallback),
...
pub fn handle_start_subtitle_narration(
    ...
    let narration_language_code = detect_narration_job_language(&request.items);
...
fn detect_narration_job_language(items: &[SubtitleNarrationItemRequest]) -> Option<String> {
    let sample = items
        .iter()
        .filter_map(|item| {
...
fn run_subtitle_narration(
    ...
    for (index, item) in request.items.iter().enumerate() {
        ...
        match &result {
            Ok((path, duration)) => eprintln!(
                "[Narration][job={}] item {}/{} subtitle_id={} OK duration_sec={:.3} elapsed_ms={} path={}",
                job_id,
                index + 1,
                total,
                item.id,
            ...
            Err(message) => eprintln!(
                "[Narration][job={}] item {}/{} subtitle_id={} FAILED elapsed_ms={} error={}",
                job_id,
                index + 1,
                total,
                item.id,
        ...
        let _ = update_snapshot(&snapshot, |state| {
            ...
            state.progress = state.completed_items as f64 / total.max(1) as f64;
            match result {
                Ok((path, duration)) => state.results.push(SubtitleNarrationResult {
