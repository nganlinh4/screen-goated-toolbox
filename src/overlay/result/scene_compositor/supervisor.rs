...
pub(crate) fn restart_and_wait(timeout: Duration) -> bool {
    recreate_and_wait(timeout, false)
}

pub(crate) fn recover_after_failure_and_wait(timeout: Duration) -> bool {
    recreate_and_wait(timeout, true)
}

fn recreate_and_wait(timeout: Duration, force_failure: bool) -> bool {
    let previous_generation = GENERATION.load(Ordering::SeqCst);
    if force_failure {
        fail_live_renderer("forced recovery verification", true);
    } else {
