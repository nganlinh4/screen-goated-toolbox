...
fn validate_meshes(
    ...
    let mut nonindexed_vertex_total = 0_u64;
    ...
    for mesh in meshes {
        ...
        for primitive in primitives {
            ...
            let attributes = primitive
                ...
                .ok_or_else(invalid)?;
            let position_index = accessor_index(attributes.get("POSITION"), accessors)?;
            let position = accessors[position_index];
            ...
            if position_accessors.insert(position_index) {
            ...
            if let Some(value) = primitive.get("indices") {
                ...
                || !position.count.is_multiple_of(vertices_per_element)
            {
                return Err(invalid());
            } else {
                nonindexed_vertex_total = nonindexed_vertex_total
                    .checked_add(position.count)
                    .filter(|total| *total <= MAX_GLTF_VERTICES)
                    .ok_or_else(invalid)?;
...
fn valid_vertex_attribute(semantic: &str, accessor: AccessorInfo) -> bool {
    ...
    match semantic {
        ...
        "WEIGHTS_0" => accessor.component_count == 4 && valid_float_or_normalized_integer(accessor),
...
fn accessor_at(value: Option<&Value>, accessors: &[AccessorInfo]) -> Result<AccessorInfo, String> {
    accessor_index(value, accessors).map(|index| accessors[index])
}

fn accessor_index(value: Option<&Value>, accessors: &[AccessorInfo]) -> Result<usize, String> {
    value
        .and_then(Value::as_u64)
        .and_then(|index| usize::try_from(index).ok())
        .filter(|index| *index < accessors.len())
...
mod tests {
    ...
    #[test]
    fn indexed_parts_share_one_bounded_vertex_store() {
        ...
        let root = serde_json::json!({
            ...
            "nodes": [{"mesh": 0}, {"mesh": 1}, {"children": [0, 1]}],
            "scenes": [{"nodes": [2]}]
        });
        assert!(validate(root.as_object().unwrap(), &[position, indices]).is_ok());
    }
}
