...
fn buffer_table_and_unused_viewer_features_are_bounded() {
    ...
    assert!(features::validate(root.as_object().unwrap()).is_ok());
    assert!(skins::validate(root.as_object().unwrap(), &[accessor], &[], &[]).is_err());
    assert!(semantics::validate(root.as_object().unwrap(), &[accessor]).is_ok());
}
...
pub(super) fn glb_bytes(json: &[u8], binary: &[u8]) -> Vec<u8> {
