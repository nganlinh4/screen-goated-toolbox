...
pub(in crate::overlay::image_creator) fn runtime_preparation_status() -> String {
    if super::runtime_command().is_none() {
...
fn refresh_preparation_status() -> bool {
    let Some(status) = crate::overlay::creation_runtime::query_preparation_status("image") else {
        return true;
    };
    *PREPARATION_STATUS
        .lock()
...
fn retry_delay(failure_streak: u32) -> Duration {
    Duration::from_secs((u64::from(failure_streak.max(1)) * 5 * 60).min(15 * 60))
}
...
fn run_warm_batch(count: usize) -> (usize, usize) {
    ...
    for _ in 0..count {
        warmers.push(std::thread::spawn(|| {
            let Some(mut command) = super::runtime_command() else {
                return false;
            };
            command
                .arg("--warm-image-headless")
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .stdin(Stdio::null());
    ...
    loop {
        if super::runtime_command().is_none() {
            ...
            if let Err(error) = installed {
                ...
                crate::log_info!(
                    "[Image creator] Creation tools install failed; retrying in {}s: {error}",
                    delay.as_secs()
                );
        ...
        if failures > 0 {
            ...
            let delay = retry_delay(failure_streak);
            crate::log_info!(
                "[Image creator] {failures} preparation attempt(s) failed; retrying in {}s",
                delay.as_secs()
            );
            wait_for_maintenance(delay);
        } else {
...
fn start_maintainer(install_if_missing: bool) {
    if install_if_missing {
        INSTALL_ALLOWED.store(true, Ordering::SeqCst);
    } else if super::runtime_command().is_none() {
        return;
    }
...
mod tests {
    ...
    #[test]
    fn preparation_is_serial_and_stops_when_complete() {
