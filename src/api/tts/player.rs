...
use super::utils::{clear_tts_state, clear_tts_loading_state};
...
impl AudioPlayer {
    fn new(sample_rate: u32, manager: Arc<TtsManager>) -> Self {
        ...
        let thread = std::thread::spawn(move || {
            ...
            let result = Self::create_excluded_stream(sample_rate, buffer_clone.clone(), shutdown_clone.clone(), target_device_id, manager);
    ...
    fn create_excluded_stream(
        ...
        std::thread::spawn(move || {
            if let Err(e) = unsafe { Self::run_wasapi_excluded(_sample_rate, buffer_clone.clone(), shutdown_clone.clone(), target_device_id, manager) } {
    ...
    fn play(&self, audio_data: &[u8], is_realtime: bool) {
        ...
        if input_samples.is_empty() { return; }
