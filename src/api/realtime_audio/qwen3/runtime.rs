...
fn validate_probe_capabilities(probe: &ProbeResponse, requested_kv_cache_mode: &str) -> Result<()> {
    ...
    if probe_kv_cache_mode != requested_kv_cache_mode {
        let message = format!(
            "Qwen3 TurboQuant runtime reported kv_cache_mode '{}' but the app requested '{}'.",
            probe.kv_cache_mode, requested_kv_cache_mode
        );
        set_runtime_notice(&message);
        return Err(anyhow!(message));
    }
