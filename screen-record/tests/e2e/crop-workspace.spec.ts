...
test("crop wheel zoom magnifies the view without changing the crop", async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 1280, height: 800 });
  await openCropWorkspace(page);
  await page.getByRole("button", { name: "Aspect ratio 9:16, 406×720" }).click();
  await page.getByRole("button", { name: "Unlock aspect ratio" }).click();
  const video = page.locator(".crop-workspace-video");
  const readout = page.locator(".crop-ratio-selection-size");
  const original = await video.boundingBox();
  expect(original).not.toBeNull();
  if (!original) return;
  const eastHandle = page.locator(".crop-workspace-handle.cursor-e-resize");
  const handle = await eastHandle.boundingBox();
  expect(handle).not.toBeNull();
  if (!handle) return;
  const anchor = { x: handle.x + handle.width / 2, y: handle.y + handle.height / 2 };
  await page.mouse.move(anchor.x, anchor.y);
  await page.mouse.wheel(0, -350);
  await expect.poll(async () => (await video.boundingBox())!.width).toBeGreaterThan(original.width * 1.9);
  await expect(readout).toHaveText("406 × 720");
  const zoomed = (await video.boundingBox())!;
  expect((anchor.x - zoomed.x) / zoomed.width).toBeCloseTo((anchor.x - original.x) / original.width, 2);
  const zoomedHandle = (await eastHandle.boundingBox())!;
  await page.mouse.move(zoomedHandle.x + zoomedHandle.width / 2, zoomedHandle.y + zoomedHandle.height / 2);
  await page.mouse.down();
  await page.mouse.move(zoomedHandle.x + zoomedHandle.width / 2 - 40, zoomedHandle.y + zoomedHandle.height / 2);
  await page.mouse.up();
  const dimensions = (await readout.innerText()).match(/(\d+) × (\d+)/)!;
  expect(Number(dimensions[1])).toBeCloseTo(406 - 40 / zoomed.width * 1280, -1);
  expect(Number(dimensions[2])).toBe(720);
  await page.screenshot({ path: testInfo.outputPath("crop-zoom-dark.png") });
  await page.evaluate(() => window.postMessage({ type: "sr-set-settings", theme: "light" }, "*"));
  await expect(page.locator("html")).not.toHaveClass(/dark/);
  await page.screenshot({ path: testInfo.outputPath("crop-zoom-light.png") });
  await page.mouse.wheel(0, 600);
  await expect.poll(async () => (await video.boundingBox())!.width).toBeCloseTo(original.width, 0);
  await page.mouse.wheel(0, -200);
  await page.locator(".crop-workspace-cancel-btn").click();
  await page.locator(".playback-crop-toggle-btn").click();
  await expect.poll(async () => (await video.boundingBox())!.width).toBeCloseTo(original.width, 0);
});
