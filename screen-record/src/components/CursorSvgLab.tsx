...
const SGTWATERMELON_ITEMS: CursorItem[] = CURSOR_TYPES.map((t) => ({
  key: `sgtwatermelon-${t.id}`,
  label: `SGT Watermelon • ${t.label}`,
  src: `/cursor-${t.id}-sgtwatermelon.svg`,
}));

const SGTVEGGIE_ITEMS: CursorItem[] = CURSOR_TYPES.map((t) => ({
  key: `sgtveggie-${t.id}`,
  label: `SGT Veggie • ${t.label}`,
...
const SGTFASTFOOD_ITEMS: CursorItem[] = CURSOR_TYPES.map((t) => ({
  ...
  src: `/cursor-${t.id}-sgtfastfood.svg`,
}));

const SGTKOREA_ITEMS: CursorItem[] = CURSOR_TYPES.map((t) => ({
  key: `sgtkorea-${t.id}`,
  label: `SGT Korea • ${t.label}`,
  src: `/cursor-${t.id}-sgtkorea.svg`,
}));
