...
export function SequencePillChain({
  ...
  return (
    ...
    >
      <div className="sequence-pill-chain flex min-w-0 flex-1 items-center justify-center overflow-x-auto py-1">
        <div className="flex min-w-max items-center gap-1.5">
          <button
            ...
            className="sequence-pill-add-btn flex h-7 w-7 items-center justify-center rounded-full border border-[var(--overlay-divider)] bg-[var(--surface-container)] text-[var(--primary-color)] transition-colors hover:bg-[var(--glass-bg)] hover:text-[var(--on-surface)]"
              ...
              <div key={clip.id} className="flex items-center gap-1.5">
                <div className="relative">
                  <button
                    ...
                    className={`sequence-pill group relative flex min-w-[7.5rem] max-w-[10rem] items-center gap-2 rounded-full border px-3 py-1.5 text-left transition-colors ${
                      isSelected
                        ? "border-[var(--primary-color)] bg-[var(--primary-color)] text-white shadow-[0_6px_18px_rgba(0,0,0,0.12)]"
                        : "border-[var(--overlay-divider)] bg-[var(--surface-container)] text-[var(--on-surface)] hover:bg-[var(--glass-bg)]"
                  ...
                  {clip.role !== "root" && (
                    <button
                      ...
                      className={`sequence-pill-remove absolute -right-1 -top-1 flex h-[18px] w-[18px] items-center justify-center rounded-full border transition-colors ${
                        isSelected
                          ? "border-white/30 bg-black/35 text-white hover:bg-red-500"
                          : "border-[var(--overlay-divider)] bg-[var(--surface)] text-[var(--on-surface-variant)] hover:bg-red-500 hover:text-white"
                      }`}
                ...
                {isMultiClip && index < composition.clips.length - 1 && (
                  <button
                    ...
                    className="sequence-pill-gap relative flex h-7 w-7 items-center justify-center rounded-full border border-transparent text-[var(--on-surface-variant)] transition-colors hover:border-[var(--overlay-divider)] hover:bg-[var(--surface-container)] hover:text-[var(--primary-color)]"
      ...
      {isMultiClip && (
        <div className="sequence-mode-toggle-inline flex rounded-md border border-[var(--overlay-divider)] overflow-hidden">
          {(["separate", "unified"] as const).map((mode) => {
            ...
            return (
              <button
                ...
                className={`sequence-mode-toggle-btn sequence-mode-toggle-btn-${mode} px-2.5 py-0.5 text-[10px] font-semibold transition-colors ${
                  isActive
