...
internal fun validateGlbMeshes(
    ...
    val chargedPositionAccessors = mutableSetOf<Int>()
    var primitiveCount = 0
    var vertices = 0L
    var nonindexedVertices = 0L
    var indices = 0L
    var morphTargets = 0
    ...
    return List(meshes.length()) { meshIndex ->
        ...
        repeat(primitives.length()) { primitiveIndex ->
            ...
            if (chargedPositionAccessors.add(positionIndex)) {
            ...
            val indexCount = if (primitive.has("indices")) {
                ...
                nonindexedVertices = checkedAdd(nonindexedVertices, position.count)
            ...
            require(
                vertices <= CREATION_GLB_MAXIMUM_VERTICES &&
                    nonindexedVertices <= CREATION_GLB_MAXIMUM_VERTICES &&
...
private fun validCreationGlbVertexAttribute(
    ...
    "JOINTS_0" ->
        accessor.type == "VEC4" &&
            accessor.componentType in setOf(5_121, 5_123) &&
            !accessor.normalized
    "WEIGHTS_0" -> accessor.type == "VEC4" && accessor.isFloatOrNormalizedInteger()
