...
internal class CreationOutputStore(
    ...
    private val safDelivery = CreationSafDeliveryStore(resolver, safTrees)
    ...
    fun defaultDirectoryLabel(tool: CreationTool): String = when {
        tool.usesProjectLibrary() -> DEFAULT_LABEL
        outputTree() != null -> directoryLabel(requireNotNull(outputTree()))
        tool.usesDownloadsByDefault() -> DOWNLOADS_LABEL
        else -> DEFAULT_LABEL
    }
    fun destinationSnapshot(): String? = outputTree()?.toString()
    fun destinationSnapshot(tool: CreationTool): String? =
        if (tool.usesProjectLibrary()) null
        else destinationSnapshot() ?: downloads.destination.takeIf { tool.usesDownloadsByDefault() }
    fun availableBytes(destination: String): Long? {
        if (destination == downloads.destination) return downloads.availableBytes()
        return runCatching {
    ...
    fun plan(
        ...
        } else if (destination == downloads.destination) {
            val reservationToken = UUID.randomUUID().toString().replace("-", "")
            val occupied: (String) -> Boolean = { name ->
                name in reserved || downloads.find(name) != null
            }
            val finalName = uniqueCreationDownloadsName(requested, occupied, dispatchId)
            val pendingName = creationDownloadsPendingName(reservationToken, finalName)
            require(!occupied(pendingName)) { "Creation reservation is already occupied" }
            CreationPublishIntent(
                kind = DOWNLOADS_KIND,
    ...
    fun reserve(
        ...
        return when (intent.kind) {
            ...
            DOWNLOADS_KIND -> downloads.reserve(intent)
            SAF_KIND -> safDelivery.reserve(intent)
    ...
    fun populate(
        ...
        when (intent.kind) {
            ...
            DOWNLOADS_KIND -> downloads.populate(intent, pendingHandle, source)
            SAF_KIND -> safDelivery.populate(intent, pendingHandle, source)
            else -> error("Unsupported creation destination")
        }
    ...
    fun commit(
        ...
        DOWNLOADS_KIND -> commitDownloads(
            intent, pendingHandle, pendingIdentity, expectedSize, expectedSha256,
        )
        SAF_KIND -> safDelivery.commit(
            intent, pendingHandle, pendingIdentity, expectedSize, expectedSha256,
        )
        else -> error("Unsupported creation destination")
    }
    ...
    fun abort(
        ...
        DOWNLOADS_KIND -> {
            ...
            downloads.owns(pending) &&
                (!pathExists(pendingHandle) || pendingHandle == pendingIdentity) &&
                (!pathExists(pendingHandle) || downloads.delete(pending))
        }
        SAF_KIND -> {
            val pending = Uri.parse(pendingHandle)
    ...
    fun planHistoryRenameNames(
        ...
        if (uri != null && downloads.owns(uri)) {
            val currentName = downloads.displayName(uri)
            val companionUri = companionPath?.creationContentUri()
            val currentCompanionName = companionUri?.let(downloads::displayName)
            return creationHistoryRenameNames(
                safeCreationOutputName(requestedName),
                companionName,
                { name -> name != currentName && downloads.find(name) != null },
                { name -> name != currentCompanionName && downloads.find(name) != null },
                transactionId,
            )
        }
        return creationHistoryRenameNames(
    ...
    fun renameForHistory(
        ...
        if (downloads.owns(uri)) {
            downloads.find(targetName)?.let { existing ->
                require(artifactIdentity(existing.toString()) == expectedIdentity)
            ...
            val published = downloads.rename(uri, targetName)
    ...
    private fun commitDownloads(
        ...
        downloads.find(intent.finalName)?.let { published ->
            require(
        ...
        require(artifactIdentity(pendingHandle) == pendingIdentity)
        require(verifiedUri(Uri.parse(pendingHandle), expectedSize, expectedSha256)) {
            "Creation output verification failed"
        }
        val published = downloads.publish(intent, pendingHandle)
        require(artifactIdentity(published) == pendingIdentity)
        require(verifiedPath(published, expectedSize, expectedSha256)) {
            "Creation output verification failed"
        }
        return published
    }
    ...
    private fun verifiedUri(uri: Uri, size: Long, digest: String): Boolean =
        runCatching {
            ...
            (observedSize == null || observedSize == size) &&
                resolver.openInputStream(uri)?.use {
                    creationStreamMatchesProof(it, size, digest)
                } == true
        }.getOrDefault(false)
    ...
    private fun publishedHandle(intent: CreationPublishIntent): String? = when (intent.kind) {
        ...
        DOWNLOADS_KIND -> downloads.find(intent.finalName)?.toString()
    ...
    private fun artifactIdentity(path: String): String? =
        path.creationContentUri()?.let { uri ->
            if (downloads.owns(uri)) return@let uri.toString()
            runCatching {
    ...
    private fun deleteExactPath(path: String): Boolean =
        path.creationContentUri()?.let { uri ->
            runCatching {
                if (downloads.owns(uri)) downloads.delete(uri)
    ...
    private companion object {
        ...
        const val DOWNLOADS_LABEL = "Downloads"
        const val MANAGED_KIND = "managed"
        const val DOWNLOADS_KIND = "downloads"
