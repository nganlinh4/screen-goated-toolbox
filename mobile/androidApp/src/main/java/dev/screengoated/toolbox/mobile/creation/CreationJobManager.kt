...
import dev.screengoated.toolbox.mobile.creation.worker.creationRequiredWorkerKey
import java.io.File
...
internal class CreationJobManager internal constructor(context: Context) {
    ...
    private val surfaces = CreationSurfaceWorkerCoordinator(workers)
    ...
    fun acquireSurface(tool: CreationTool, ownerId: String) = surfaces.acquire(tool, ownerId)
    fun releaseSurface(tool: CreationTool, ownerId: String) = surfaces.release(tool, ownerId)
    ...
    fun startJob(
        ...
        val destination = files.outputDestinationSnapshot(tool)
        ...
        recoveryLeases.acquire(jobId, tool, creationRequiredWorkerKey(tool, request))
        diagnostics.event(
            "job_queued",
            tool.wireName,
            jobId = jobId,
