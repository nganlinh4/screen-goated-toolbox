...
private fun validateGlbUnsupportedWork(document: JSONObject) {
    ...
    repeat(nodes.length()) { index ->
        require(!nodes.requiredObject(index).has("camera")) {
            "The model result contains unsupported runtime work"
        }
    }
}
...
internal const val CREATION_GLB_MAXIMUM_SKINS = 64
internal const val CREATION_GLB_MAXIMUM_JOINTS_PER_SKIN = 512
internal const val CREATION_GLB_MAXIMUM_TOTAL_JOINTS = 4_096
