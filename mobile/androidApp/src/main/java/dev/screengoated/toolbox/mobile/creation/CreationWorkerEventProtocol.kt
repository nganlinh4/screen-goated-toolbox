...
internal fun creationWorkerCanServeFollowUp(event: CreationWorkerEvent): Boolean =
    event.event == "success" && (
        event.canSegment == true ||
            event.canRefine == true ||
            event.availableActions?.isNotEmpty() == true
        )
