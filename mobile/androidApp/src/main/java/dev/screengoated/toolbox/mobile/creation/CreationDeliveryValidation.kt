...
internal fun validCreationDeliveryRecord(
    ...
    val intentValid = token != null &&
        ...
        when (record.intent.kind) {
            ...
            "downloads" -> record.intent.destination == CREATION_DOWNLOADS_DESTINATION &&
                record.intent.targetPath == null &&
                record.intent.pendingPath == null &&
                record.intent.pendingName == creationDownloadsPendingName(
                    token,
                    record.intent.finalName,
                )
    ...
    val companionValid = record.companion?.let { companion ->
        ...
        val companionIntentValid = companionToken != null &&
            ...
            when (companion.intent.kind) {
                ...
                "downloads" -> companion.intent.destination == CREATION_DOWNLOADS_DESTINATION &&
                    companion.intent.targetPath == null &&
                    companion.intent.pendingPath == null &&
                    companion.intent.pendingName == creationDownloadsPendingName(
                        companionToken,
                        companion.intent.finalName,
                    )
