...
class LiveTranslateOverlayBootstrapTest {
    ...
    fun `active Android model picker labels stay human-readable`() {
        ...
        assertEquals("(Transcribe) 2.5 Live", transcriptionOptions.first { it.id == RealtimeModelIds.TRANSCRIPTION_GEMINI_2_5 }.label)
        assertEquals("(Transcribe) 3.1 Live", transcriptionOptions.first { it.id == RealtimeModelIds.TRANSCRIPTION_GEMINI_3_1 }.label)
        assertEquals("(Translate) 3.5 Translate", transcriptionOptions.first { it.id == RealtimeModelIds.TRANSCRIPTION_GEMINI_TRANSLATE }.label)
        assertEquals("Parakeet (Unavailable)", transcriptionOptions.first { it.id == RealtimeModelIds.TRANSCRIPTION_PARAKEET }.label)
        assertEquals("(Transcribe) Moonshine Tiny", transcriptionOptions.first { it.id == "moonshine-tiny-streaming" }.label)
        assertEquals("(Transcribe) Moonshine Small", transcriptionOptions.first { it.id == "moonshine-small-streaming" }.label)
        assertEquals("(Transcribe) Moonshine Medium", transcriptionOptions.first { it.id == "moonshine-medium-streaming" }.label)
        assertEquals("(Transcribe) Zipformer", transcriptionOptions.first { it.id == "zipformer" }.label)

        val mainJsSource = loadRepoFile(OVERLAY_MAIN_JS_PATH).readText() +
            loadRepoFile(OVERLAY_MAIN_JS_PART2_PATH).readText()
        assertTrue(mainJsSource.contains("'gemini-live-audio': '(Transcribe) 2.5 Live'"))
        assertTrue(mainJsSource.contains("'gemini-live-audio-3.1': '(Transcribe) 3.1 Live'"))
        assertTrue(mainJsSource.contains("'gemini-3.5-translate': '(Translate) 3.5 Translate'"))
        assertTrue(mainJsSource.contains("'moonshine-tiny-streaming': '(Transcribe) Moonshine Tiny'"))
        assertTrue(mainJsSource.contains("'moonshine-small-streaming': '(Transcribe) Moonshine Small'"))
        assertTrue(mainJsSource.contains("'moonshine-medium-streaming': '(Transcribe) Moonshine Medium'"))
        assertTrue(mainJsSource.contains("'zipformer': '(Transcribe) Zipformer'"))
