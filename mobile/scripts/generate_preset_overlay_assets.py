...
def extract_match_arm_svg(source_file: Path, arm_name: str) -> str:
    source = read(source_file)
    pattern = re.compile(
    ...
    include_match = include_pattern.search(source)
    if include_match is not None:
        return read(source_file.parent / include_match.group(1))

    raise ValueError(f"Missing SVG match arm: {arm_name}")
...
def generate(args: argparse.Namespace) -> None:
    ...
    recording_template = recording_template.replace(
        "{{ICON_PAUSE}}", extract_match_arm_svg(args.icons_source, "pause")
    )
    recording_template = recording_template.replace(
        "{{ICON_PLAY}}", extract_match_arm_svg(args.icons_source, "play_arrow")
    )
    recording_template = recording_template.replace(
        "{{ICON_CLOSE}}", extract_match_arm_svg(args.icons_source, "close")
    )
