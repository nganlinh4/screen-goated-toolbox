...
derive their public version from it, but several surfaces repeat it and do not
...
| `component-delivery/windows/creation-v1.json` | `hostVersion` |
...
Run the pin checker before building. It scans `component-delivery/`, so future
manifests are covered without extending the table above.
...
Localization parity is mandatory. The `github` and `announcement vi-VN`
sections must describe the same product changes in the same order,
bullet-for-bullet; only the language, announcement preface, and GitHub footer
may differ. Every `play <locale>` section must be a semantic translation of the
same compact source, with no locale-specific additions or omissions. If a store
limit requires shorter copy, shorten every locale together before translating.

Play release notes are Android-only. Include only changes that users receive in
the Android Play artifact or its shared cross-platform product behavior. Never
mention Windows, PC, desktop-only UI, EXE packaging, tray behavior, WebView2, or
any other change absent from the Play build. Platform-specific Windows changes
belong only in the GitHub and announcement sections.
...
comparisons, binary-ownership reports, Android macrobenchmarks, browser/provider
investigations, UI harnesses, labs, or evidence-capture tools as part of a
release or canonical build. Those tools belong only to the implementation and
diagnosis workflows in `docs/DEVELOPMENT.md`.

Release validation is limited to facts that can change during publication:

1. version pins and release notes match the reviewed source;
2. every changed optional component is packaged once from that source and its
   immutable published bytes are read back and matched exactly;
3. staging contains no candidate selected by a production build;
4. Windows and Android canonical artifacts are built once; and
5. the exact final EXE/APK artifacts receive a bounded launch/install smoke.

Do not use a release request to reopen completed development investigations or
to compare build profiles. The canonical Windows artifact always uses the
tracked `release` profile.
...
build the separately tracked runtime first. Android consumes its platform
runtime distributions directly; Windows packages that exact runtime together
with the Creation interface as one product archive. Update either tracked
contract only after remote read-back. Each capability is active only when its
release-availability fixture enables it.
...
1. Rebuild the Windows creation runtime and the combined Windows Creation
   archive when either the engine or interface bytes can change.
...
6. Commit the verified Android/runtime authority at
   `component-delivery/creation-runtime-v1.json` and the verified physical
   Windows product at `component-delivery/windows/creation-v1.json`.

Creation runtime source and its build procedure are maintained in the private
[`sgt-creation-runtime`](https://github.com/nganlinh4/sgt-creation-runtime)
repository. Follow that repository's release instructions; this public host
accepts only the resulting reviewed, immutable delivery contract.

```powershell
.\scripts\build-component-candidate.ps1 -Component creation -Stage
...
The same runtime version and feature handshake must feed Windows, Android Full,
and Android Play. Tracked contracts are build inputs, so ordinary and canonical
builds resolve identical immutable locations and integrity metadata. Missing or
invalid delivery data is a compile-time failure, never a local fallback.
...
PromptDJ and TTS Playground interfaces ship as optional,
removable Windows components rather than bytes embedded in the desktop host.
Complete this checkpoint before `build.ps1`:

1. Build both reviewed frontends and create deterministic ZIP packages.
...
`build.ps1` never builds or embeds these native packages. It downloads and
hashes every immutable asset named by the tracked delivery contract, then fails
before compiling the host if the published bytes do not match. Rebuild the
packages only when their source or inventory changes.

### Mandatory Screen Recorder checkpoint

Screen Recorder ships as one independently removable x64 component containing
the standalone native worker and its frontend. Reproduce the content-addressed
package, upload only that new immutable ZIP, then verify the remote bytes:

```powershell
.\scripts\build-recorder-component-packs.ps1 `
  -OutputDir (Join-Path $componentPackageRoot "sgt_recorder") `
  -CargoTargetDir $packageCargoTarget
# Upload only the new ZIP listed in sgt_recorder.packages.json.
...
The recorder package includes the exact worker and frontend third-party license
inventories. The host build fails closed when the remote package is missing or
differs from the read-back-verified size, SHA-256, or file inventory. Never
replace an existing runtime-bundles asset.
...
tag. It downloads the public Windows executable and Full Android APK, verifies
GitHub's size and SHA-256 for both, builds a signed manifest, and advances the
`app-update-feed` branch with one
...
semantic versions, the exact versioned Windows and Full Android asset names and
GitHub URLs, and matching sizes and SHA-256 values. If the feed has not been created yet, the client uses
