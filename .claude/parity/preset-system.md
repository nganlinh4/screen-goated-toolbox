...
- Vision payloads preserve their real MIME type. Groq images use a prompt-aware encoded-byte budget below the provider request ceiling: keep PNG when it fits, otherwise use adaptive JPEG compression and resizing before sending. Qwen vision uses 2,048 maximum completion tokens; other Groq vision models leave the ceiling unset. Qwen reasoning stays at provider default with `reasoning_format: hidden`; only when a completed response contains no final content does the client retry once with reasoning disabled. A token-rate 429 may retry once when Groq's structural `retry-after` is at most 30 seconds; otherwise preserve the provider error and continue the normal fallback chain. Windows and Android use the same contract.
