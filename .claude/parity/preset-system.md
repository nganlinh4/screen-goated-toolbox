...
- Vision payloads preserve their real MIME type. Groq images use a prompt-aware encoded-byte budget below the provider request ceiling: keep PNG when it fits, otherwise use adaptive JPEG compression and resizing before sending. Qwen vision uses 4,096 maximum completion tokens, leaving room below its 8K TPM limit for image/input tokens; other Groq vision models leave the ceiling unset. Qwen reasoning stays at provider default but uses `reasoning_format: hidden`, so only final content reaches users. Windows and Android use the same contract.
