...
- Vision payloads preserve their real MIME type. Groq images use a prompt-aware encoded-byte budget below the provider request ceiling: keep PNG when it fits, otherwise use adaptive JPEG compression and resizing before sending. Qwen vision uses 1,024 maximum completion tokens because Groq reserves the requested output against its 8K TPM limit; other Groq vision models leave the ceiling unset. Qwen reasoning stays at provider default but uses `reasoning_format: hidden`, so only final content reaches users. A token-rate 429 may retry once when Groq's structural `retry-after` is at most 30 seconds; otherwise preserve the provider error and continue the normal fallback chain. Windows and Android use the same contract.
