...
- Transition rules:
  ...
  - The Android update **source and action depend on the distribution flavor**:
    ...
    - `full` (sideload) flavor: GitHub-driven (`androidApp/src/full/.../updater/AppUpdateRepository.kt`) — prefer the latest `.apk` asset URL; fall back to the release page URL when no `.apk` exists. All GitHub release and package-selection code is confined to this flavor source set.
- Output contract:
