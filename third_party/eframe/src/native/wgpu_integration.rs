...
impl WgpuWinitRunning<'_> {
    ...
    fn on_window_event(
        ...
        // A normal Windows surface needs an immediate frame while it is being resized. A
        // DirectComposition surface with reserved capacity does not: rendering every intermediate
        // size synchronously blocks delivery of newer resize messages and makes the window trail the
        // pointer. Queue one redraw instead so winit can collapse a burst to its latest dimensions.
        ...
        match event {
            ...
            winit::event::WindowEvent::Resized(physical_size) => {
                    ...
                    )
                {
                    ...
                    repaint_asap = !cfg!(target_os = "windows");
                }
            }
