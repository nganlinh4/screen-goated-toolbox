...
pub enum WgpuError {
    ...
    // SGT modification: reports failures while rebuilding Painter GPU state after device loss.
    #[cfg(feature = "winit")]
    #[error("Could not recover the wgpu device: {0}")]
...
pub struct WgpuConfiguration {
    ...
    /// Called when device-loss recovery is detected, succeeds, or is retried.
    ///
    /// SGT modification: this lets a release application persist recovery diagnostics without
    /// installing a global logger for every dependency.
    pub on_device_recovery: Arc<dyn Fn(&str) + Send + Sync>,
}
...
impl Default for WgpuConfiguration {
    fn default() -> Self {
        Self {
            ...
            on_device_recovery: Arc::new(|message| log::info!("{message}")),
        }
    }
}
