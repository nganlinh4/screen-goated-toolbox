...
impl Surface {
    ...
    pub fn uses_window_composition_visual(&self) -> bool {
        ...
        let swap_chain = match self.swap_chain.write().take() {
            ...
            None => {
                let desc = Dxgi::DXGI_SWAP_CHAIN_DESC1 {
                    ...
                    // Stretching an HWND swap chain makes DXGI distort the last
                    // presented frame while interactive resizing outruns rendering.
                    // Keep composition targets unchanged; their sizing contract is
                    // controlled by the composition visual rather than an HWND.
                    Scaling: if matches!(self.target, SurfaceTarget::WndHandle(_)) {
                        Dxgi::DXGI_SCALING_NONE
                    } else {
